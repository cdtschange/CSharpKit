...
{
    public class Mail_Yeahnet : MailBase
    {
        public const string login_ssl = @"https://ssl.mail.yeah.net/entry/cgi/ntesdoor?funcid=loginone&language=-1&passtype=1&verifycookie=1&iframe=1&from=web&style=-1&rnd={0}_{1}@yeah.net&times=1&df=index";
        public const string login_ssl_data = @"bCookie=&username={0}@yeah.net&product=mailyeah&savelogin=0&style=-1&url2=http%3A%2F%2Fmail.yeah.net%2Ferrorpage%2Ferr_yeah.htm&user={0}&password={1}";


        public const string password_url = @"http://reg.163.com/setinfo/ChangePwd_1.jsp?noheader=1&username={0}@yeah.net&sid={1}&uid=ddmailyeah@yeah.net&host={2}.mail.yeah.net&ver=js4&style=1&skin=yeahblue&color=005590";
        public const string password_url_submit = @"https://reg.163.com/setinfo/ChangePasswd_2.jsp";
        public const string password_url_data = @"username={0}@yeah.net&uname2={5}&cos=&syscheckcode={4}&noheader=1&oldpass={1}&password={2}&confirmPassword={2}&usercheckcode={3}&submit=%E4%B8%8B%E4%B8%80%E6%AD%A5";
        ...
        public const string openPOP3_url_submit = @"http://config.mail.yeah.net/pop3setting/aj.jsp?rnd=0.18457492738820735";
        public const string openPOP3_url_data = @"uid={0}@yeah.net&sid={1}&action=set&pop3config=on&imapconfig=on&smtpconfig=on";

        public const string autoforward_url_submit = @"http://config.mail.yeah.net/autofw/fwto.do?sid={0}&forwarddes={1}&keeplocal=0&callback=netease.mail.base.Ajax.padding1310653180998";
        public const string cancelforward_url = @"http://{0}.mail.yeah.net/js3/s?sid={1}&func=user:getAttrs&requestfunc=global:sequential&requesttime=239";
        ...
        public const string cancelforward_url_submit = @"http://{0}.mail.yeah.net/js3/s?sid={1}&func=user:setAttrs&requestfunc=user:getAttrs&requesttime=176";
        ...
        public const string reg_account_url_submit_data = @"uname={0}&password={1}&passwordconf={1}&mobile=&verifycode={2}&domain=yeah.net&version=regvf1";
        ...
        public Mail_Yeahnet(MailModel model)
            : base(model)
        {
        ...
        public override bool Login()
        {
            ...
            try
            {
                ...
                html = httpClient.SendPost(string.Format(login_ssl, Common.GetServerTime(13), model.Account), Encoding.GetEncoding("GB2312").GetBytes(data), Encoding.GetEncoding("GB2312"), ref cookie, out ErrorMessage);
                ...
                html = httpClient.SendGet(url, Encoding.UTF8, ref cookie, out ErrorMessage);
                if (!Regex.Match(html, @"正在登录").Success)
                {
                    ErrorMessage = string.Format("账号：{0}登录失败，错误原因：解析匹配失败！", model.FullAccount);
                    return false;
                }
        ...
        public override bool RegistAccount()
        {
            ...
            try
            {
                ...
                cookie.SetCookies(new Uri(string.Format("http://reg.email.163.com")), string.Format("_ntes_nnid={0}", s + System.Web.HttpUtility.UrlEncode(",") + "0"));
                ...
                if (!m.Success)
                {
                    ErrorMessage = string.Format("账号：{0}注册失败，Fail/Greylist错误原因：{1}！", model.FullAccount);
                    return false;
                }
                return true;
            }
        ...
        public override bool CheckRegistAccount()
        {
            ...
            try
            {
                ...
                m = Regex.Match(html, @"mailyeah:(?<account>[^,]*),");
                if (m.Success)
                {
        ...
        public override bool ChangePassword(string newPassword)
        {
            ...
            try
            {
                ...
                foreach (Cookie ck in ccList)
                {
                    cc.SetCookies(new Uri(string.Format("http://reg.163.com")), string.Format("{0}={1}", ck.Name, ck.Value));
                }

                html = httpClient.SendGet(string.Format(password_url, model.Account, this.sid, this.sub_domain), Encoding.UTF8, ref cc, out ErrorMessage);
                ...
                data = string.Format(password_url_data, model.Account, model.Password, newPassword, usercheckcode, syscheckcode, uname2);
                html = httpClient.SendPost(password_url_submit, Encoding.UTF8.GetBytes(data), Encoding.UTF8, ref cc, out ErrorMessage);
                if (!Regex.Match(html, @"跳转提示").Success)
                {
                    ErrorMessage = string.Format("账号：{0}修改密码失败，错误原因：解析匹配失败！", model.FullAccount);
                    return false;
                }
        ...
        public override bool SetAutoForward(string email)
        {
            ...
            cookie.SetCookies(new Uri(string.Format("http://config.mail.yeah.net", sub_domain)), string.Format("Coremail={0}%{1}%{2}.mail.yeah.net", Common.GetServerTime(13), sid, sub_domain.Substring(1)));
            cookie.SetCookies(new Uri(string.Format("http://config.mail.yeah.net", sub_domain)), string.Format("mail_uid={0}@yeah.net", model.Account));
