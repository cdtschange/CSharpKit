...
{
    public class Mail_126com : MailBase
    {
        public const string login_ssl = @"https://ssl.mail.126.com/entry/cgi/ntesdoor?hid=10010102&funcid=loginone&language=-1&passtype=1&verifycookie=1&iframe=1&from=web&product=mail126&style=-1&race=5018_349_465&net=t&df=index";
        public const string login_ssl_data = @"domain=126.com&product=mail126&language=-1&bCookie=&username={0}@126.com&savelogin=1&url2=http%3A%2F%2Fmail.126.com%2Ferrorpage%2Ferr_126.htm&user={0}&password={1}&style=-1&remUser=on&secure=";


        public const string password_url = @"http://reg.163.com/setinfo/ChangePasswd_1.jsp?noheader=1&username={0}@126.com&sid={1}&uid={0}@126.com&host={2}.mail.126.com&ver=js35&style=23&skin=126green&color=158144";
        public const string password_url_submit = @"https://reg.163.com/setinfo/ChangePasswd_2.jsp";
        public const string password_url_data = @"noheader=1&username={0}@126.com&uname2={5}&cos=&oldpass={1}&password={2}&confirmPassword={2}&usercheckcode={3}&syscheckcode={4}&submit=%E4%B8%8B%E4%B8%80%E6%AD%A5";
        ...
        public const string openPOP3_url_submit = @"http://config.mail.126.com/pop3setting/aj.jsp?rnd=0.18457492738820735";
        public const string openPOP3_url_data = @"uid={0}@126.com&sid={1}&action=set&pop3config=on&imapconfig=on&smtpconfig=on";


        public const string autoforward_url_submit = @"http://config.mail.126.com/autofw/fwto.do?sid={0}&forwarddes={1}&keeplocal=0&callback=MM.autofwd.valCallback";
        public const string cancelforward_url = @"http://{0}.mail.126.com/js4/s?sid={1}&func=user:getAttrs&requestfunc=global:sequential&requesttime=239";
        ...
        public const string cancelforward_url_submit = @"http://{0}.mail.126.com/js4/s?sid={1}&func=user:setAttrs&requestfunc=user:getAttrs&requesttime=176";
        ...
        public const string reg_forward_url = @"http://reg.email.163.com/mailregAll/reg0.jsp?from=126mail";
        ...
        public const string reg_account_url_submit_data = @"uname={0}&password={1}&passwordconf={1}&mobile=&verifycode={2}&domain=126.com&version=regvf1";
        ...
        public Mail_126com(MailModel login)
            : base(login)
        {
        ...
        public override bool CheckRegistAccount()
        {
            ...
            try
            {
                ...
                m = Regex.Match(html, @"mail126:(?<account>[^,]*),");
                if (m.Success)
                {
            ...
            {
                ErrorMessage = string.Format("检测账号：{0}验证信息失败，错误原因：{1}", model.Account, e.Message);
                return false;
            }
        }
        ...
        public override bool SetAutoForward(string email)
        {
            ...
            cookie.SetCookies(new Uri(string.Format("http://config.mail.126.com", sub_domain)), string.Format("Coremail={0}%{1}%{2}.mail.126.com", Common.GetServerTime(13), sid, sub_domain.Substring(1)));
            cookie.SetCookies(new Uri(string.Format("http://config.mail.126.com", sub_domain)), string.Format("mail_uid={0}@126.com", model.Account));
